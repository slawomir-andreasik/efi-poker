...
export function RoomSwitcherDropdown({
  slug,
  projectName,
  currentRoomId,
  onClose,
}: RoomSwitcherDropdownProps) {
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span
                    className={`shrink-0 text-[10px] font-bold uppercase px-1.5 py-0.5 rounded border ${roomTypeBadge(room.roomType)}`}
                  >
                    {room.roomType === 'LIVE' ? 'Live' : 'Async'}
                  </span>
                  <span
                    className={`shrink-0 text-[10px] font-bold uppercase px-1.5 py-0.5 rounded border ${statusBadge(room.status)}`}
                  >
                ...
                <div className="mt-0.5">
                  {room.roomType === 'LIVE' && room.status === 'OPEN' && (
                    <span className="text-xs text-efi-text-tertiary flex items-center gap-1.5">
                      ...
                      {room.topic
                        ? `Round ${room.roundNumber}: ${room.topic}`
